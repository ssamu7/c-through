...
# In[6]:
def record(token, device, variable, sample_time=1):
    writer = None
    (W, H) = (None, None)
    starttime = time.time()
    frame_ind = 0
    ...
    while (True):
        ...
        if args["data"] is not None:
            ...
            obj[frame_ind][3] = int(framedatetime)
            # save obj as csv every 10 frames
            if frame_ind % 10 == 0:
                obj_df = pd.DataFrame(obj)
                obj_df.columns = ['Frame', 'Objects', 'Persons', 'DateTime']
        ...
        if time.time() - init >= sample_time:
            ...
            send(token, device, variable, result)
            init = time.time()
